...
test.describe('Operations page validation @regression', () => {
  ...
  test('I want to enter an operation and validate all items @smoke', async ({ page }) => {
    ...
    const operationIdLocator = page.locator('p.facephi-ui-label').filter({ hasText: 'ID:' });
  ...
  test('I want to validate that a denied operation has a warning', async ({ page }) => {
    const resultsPage = await page.locator('#tableBody');
    ...
    const rejectedOperationElement = rejectedRow.locator('[data-test^="operationDetail-"]').last();
    await rejectedOperationElement.focus();
    await expect(rejectedOperationElement).toBeVisible();
